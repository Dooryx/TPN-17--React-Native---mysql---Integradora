  ...
  return (
    <View style={styles.container}>
      <View style={styles.pickerContainer}>
        <Text style={styles.text}>Selecciona un jugador:</Text>
        <Picker
          selectedValue={selectedPlayer}
          onValueChange={(itemValue) => setSelectedPlayer(itemValue)}>
          <Picker.Item label="-- Seleccione un jugador --" value={null} color='white' />
          {jugadores.map((jugador) => (
            <Picker.Item key={jugador.id} label={jugador.Nombre} value={jugador} />
          ))}
        </Picker>
      </View>
      <Button title="Borrar" onPress={handleDelete} color={'#83AF9E'} />
    </View>
  );
};
